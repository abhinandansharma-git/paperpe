      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex gap-3">
            <button className="px-4 py-2 bg-white/5 hover:bg-white/10 text-white rounded-lg flex items-center gap-2"><Download className="w-4 h-4" />Export</button>
            <button className="px-5 py-2 bg-purple-500 hover:bg-purple-600 text-white rounded-lg font-semibold flex items-center gap-2"><Plus className="w-5 h-5" />Add Entry</button>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-4 gap-4 mb-8">
          <div className="bg-[#141820] rounded-xl p-5 border border-white/5">
            ...
            <div className={`text-2xl font-bold ${totalPnl >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{totalPnl >= 0 ? '+' : ''}₹{totalPnl.toLocaleString()}</div>
          </div>
          <div className="bg-[#141820] rounded-xl p-5 border border-white/5">
        ...
        <div className="flex gap-3 mb-6">
          <button className="px-4 py-2 bg-white/10 text-white rounded-lg flex items-center gap-2"><Filter className="w-4 h-4" />Filter</button>
          <button className="px-4 py-2 bg-white/5 text-slate-400 rounded-lg">All</button>
          <button className="px-4 py-2 bg-white/5 text-slate-400 rounded-lg">Winners</button>
          <button className="px-4 py-2 bg-white/5 text-slate-400 rounded-lg">Losers</button>
          <button className="px-4 py-2 bg-white/5 text-slate-400 rounded-lg">This Week</button>
        </div>
            ...
            <div key={entry.id} className="bg-[#141820] rounded-xl p-5 border border-white/5 hover:border-purple-500/30 transition-colors">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${entry.pnl >= 0 ? 'bg-emerald-500/20' : 'bg-red-500/20'}`}>
                    {entry.pnl >= 0 ? <TrendingUp className="w-6 h-6 text-emerald-400" /> : <TrendingDown className="w-6 h-6 text-red-400" />}
                  </div>
                  <div>
                    ...
                    <div className="flex items-center gap-3 text-sm text-slate-400">
                      ...
                      <span className={entry.type === 'BUY' ? 'text-emerald-400' : 'text-red-400'}>{entry.type}</span>
                      <span>Qty: {entry.qty}</span>
                    </div>
                  </div>
                </div>
                <div className="text-right">
                  <div className={`text-xl font-bold ${entry.pnl >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{entry.pnl >= 0 ? '+' : ''}₹{entry.pnl.toLocaleString()}</div>
                  <div className={`text-sm ${entry.pnl >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{entry.pnl >= 0 ? '+' : ''}{entry.pnlPct}%</div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 mb-3">
                {entry.tags.map((tag, i) => <span key={i} className="px-2 py-1 bg-purple-500/20 text-purple-400 rounded text-xs flex items-center gap-1"><Tag className="w-3 h-3" />{tag}</span>)}
              </div>
              <div className="text-slate-300 text-sm bg-white/5 rounded-lg p-3 flex items-start gap-2"><MessageSquare className="w-4 h-4 text-slate-500 mt-0.5" />{entry.notes}</div>
            </div>
          ))}
        </div>
      </main>
