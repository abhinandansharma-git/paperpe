    ...
    <div className="min-h-screen bg-[#0a0e17]">
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-white flex items-center gap-3"><Bell className="w-8 h-8 text-orange-500" />Price Alerts</h1>
            <p className="text-slate-400 mt-1">Get notified when price hits your target</p>
          </div>
          <button onClick={() => setShowCreate(true)} className="px-5 py-3 bg-orange-500 hover:bg-orange-600 text-white rounded-xl font-semibold flex items-center gap-2 transition-colors"><Plus className="w-5 h-5" />Create Alert</button>
        </div>

        {showCreate && (
          <div className="bg-[#141820] rounded-xl p-6 border border-orange-500/30 mb-8">
            <h3 className="text-lg font-semibold text-white mb-4">New Alert</h3>
            <div className="grid md:grid-cols-4 gap-4">
              <input type="text" placeholder="Symbol (e.g. NIFTY)" value={newSymbol} onChange={(e) => setNewSymbol(e.target.value.toUpperCase())} className="bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder:text-slate-500 focus:outline-none focus:border-orange-500" />
              <select value={newCondition} onChange={(e) => setNewCondition(e.target.value)} className="bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-orange-500">
                <option value="above">Price Above</option>
                <option value="below">Price Below</option>
              </select>
              <input type="number" placeholder="Target Price" value={newPrice} onChange={(e) => setNewPrice(e.target.value)} className="bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder:text-slate-500 focus:outline-none focus:border-orange-500" />
              <div className="flex gap-2">
                <button onClick={() => { setAlerts([...alerts, { id: Date.now(), symbol: newSymbol, condition: newCondition, price: parseFloat(newPrice), current: 0, status: 'active' }]); setShowCreate(false); setNewSymbol(''); setNewPrice(''); }} className="flex-1 py-3 bg-orange-500 hover:bg-orange-600 text-white rounded-lg font-medium">Create</button>
            ...
            {popularAlerts.map((alert, i) => (
              <button key={i} className="p-4 bg-white/5 hover:bg-white/10 rounded-lg text-left transition-colors group">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Plus className="w-4 h-4 text-slate-500 group-hover:text-orange-500" />
                </div>
