        ...
        <div className={`${!isPro ? 'blur-sm pointer-events-none select-none' : ''}`}>
          <div className="flex gap-2 mb-8 overflow-x-auto">
            {['bulk', 'insider', 'fii-dii', 'sector'].map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)} className={`px-5 py-2.5 rounded-lg font-medium whitespace-nowrap transition-colors ${activeTab === tab ? 'bg-yellow-500 text-black' : 'bg-white/5 text-slate-400 hover:text-white'}`}>
                ...
                <tbody>
                  {bulkDeals.map((deal, i) => (
                    <tr key={i} className="border-b border-white/5 hover:bg-white/5">
                      ...
                      <td className="px-4 py-3 text-center"><span className={`px-2 py-1 rounded text-xs font-medium ${deal.type === 'BUY' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>{deal.type}</span></td>
                ...
                <tbody>
                  {insiderTrades.map((trade, i) => (
                    <tr key={i} className="border-b border-white/5 hover:bg-white/5">
                      ...
                      <td className="px-4 py-3"><span className="px-2 py-1 rounded text-xs bg-purple-500/20 text-purple-400">{trade.relation}</span></td>
                      <td className="px-4 py-3 text-center"><span className={`px-2 py-1 rounded text-xs font-medium ${trade.type === 'BUY' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>{trade.type}</span></td>
                ...
                {sectorFlow.map((s, i) => (
                  <div key={i} className="flex items-center gap-4">
                    ...
                    <div className="flex-1 h-8 bg-white/5 rounded-lg overflow-hidden relative">
                      <div className={`absolute top-0 h-full rounded-lg ${s.fiiNet >= 0 ? 'bg-emerald-500/50 left-1/2' : 'bg-red-500/50 right-1/2'}`} style={{ width: `${Math.min(Math.abs(s.fiiNet) / 30, 50)}%` }}></div>
                    </div>
                    <span className={`w-24 text-right font-medium ${s.fiiNet >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{s.fiiNet >= 0 ? '+' : ''}₹{s.fiiNet} Cr</span>
                    <span className={`w-16 text-right text-sm ${s.change >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{s.change >= 0 ? '+' : ''}{s.change}%</span>
                  </div>
                ))}
              </div>
            </div>
